...
@pytest.mark.asyncio
async def test_book_task_enqueuer_correctly_filters_by_threshold(book_recommender_api_client_v2, task_client):
    # Given
    book_task_enqueuer_service = BookTaskEnqueuerService(book_recommender_api_client_v2, task_client)
    return_book_info = {
        "1": 0,
        "2": 1,
        "3": 6
    ...
    # Then
    assert_that(book_recommender_api_client_v2.get_already_indexed_books.call_count).is_equal_to(1)
    book_recommender_api_client_v2.get_already_indexed_books.assert_called_with([3])
    assert_that(response.tasks).is_length(1)


@pytest.mark.asyncio
async def test_book_task_enqueuer_correctly_filters_by_books_already_indexed(book_recommender_api_client_v2,
                                                                             task_client):
    # Given
    book_task_enqueuer_service = BookTaskEnqueuerService(book_recommender_api_client_v2, task_client)
    return_book_info = {
...
@pytest.mark.asyncio
async def test_task_queue_correctly_enqueues_and_returns_task_name(book_recommender_api_client_v2, task_client):
    # Given
    book_task_enqueuer_service = BookTaskEnqueuerService(book_recommender_api_client_v2, task_client)
    return_book_info = {
        "1": BOOK_POPULARITY_THRESHOLD,
...
@pytest.mark.asyncio
async def test_book_popularity_call_service_exceptions_bubble_up(book_recommender_api_client_v2, task_client):
    # Given
    book_task_enqueuer_service = BookTaskEnqueuerService(book_recommender_api_client_v2, task_client)
...
@pytest.mark.asyncio
async def test_already_indexed_book_call_exceptions_bubble_up(book_recommender_api_client_v2, task_client):
    # Given
    book_task_enqueuer_service = BookTaskEnqueuerService(book_recommender_api_client_v2, task_client)
    return_book_info = {
        "1": BOOK_POPULARITY_THRESHOLD,
    ...
    _get_book_popularity_call_returns_dict(book_recommender_api_client_v2, return_book_info)
    book_recommender_api_client_v2.get_already_indexed_books = AsyncMock(side_effect=BookRecommenderApiServerException)
...
@pytest.mark.asyncio
async def test_that_payload_full_of_ineligible_books_does_not_make_book_exists_call(book_recommender_api_client_v2,
                                                                                    task_client):
    # Given
    book_task_enqueuer_service = BookTaskEnqueuerService(book_recommender_api_client_v2, task_client)
    return_book_info = {
...
def _already_indexed_books_call_returns(book_recommender_api_client_v2, return_ids: List[int]):
