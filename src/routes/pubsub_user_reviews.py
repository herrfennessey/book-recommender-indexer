...
from src.routes.pubsub_models import IndexerResponse, PubSubMessage, PubSubUserReviewV1
from src.routes.pubsub_utils import _unpack_envelope
from src.services.book_task_enqueuer_service import (
    BookTaskEnqueuerService,
    get_book_task_enqueuer_service,
)
from src.services.user_review_service import UserReviewService, get_user_review_service
...
async def handle_pubsub_message(
    ...
    book_task_enqueuer_service: BookTaskEnqueuerService = Depends(
        get_book_task_enqueuer_service
    ),
):
    """
    ...
    for item in batch.items:
        try:
            ...
            logging.error(
                "Error converting item into PubSubUserReviewV1 object. Received: %s Error: %s",
                batch.items,
                e,
            )
    ...
    if len(items) > 0:
        user_review_service_response = (
            await user_review_service.process_pubsub_batch_message(items)
        )
        indexer_response.indexed = len(user_review_service_response.indexed)

        book_ids_to_enqueue = [item.book_id for item in items]
        try:
            book_task_enqueuer_response = (
                await book_task_enqueuer_service.enqueue_books_if_necessary(
                    book_ids_to_enqueue
                )
            )
